/**
    ...
    and Thomas H. Cormen, Charles E. Leiserson, and Ronald L.
    Rivest in "Introduction to Algorithms."
...
public:
    ...
    */
    void delete_min() {
        // DEBUG
        bool DELETE_ME_DEBUG = false;
        if (min_node->value == 12) {
            DELETE_ME_DEBUG = true;
        }
        ...
        for (auto &child : min_node->children) {
            ...
            // DEBUG
            std::cout << "Pushing child into the root list: " << child->value << std::endl;
        ...
        // DEBUG
        if (DELETE_ME_DEBUG) {
            std::cout << "Roots stored in the heap [BEFORE] delete min: " << std::endl;
            print_heap();
        }
        ...
        // DEBUG
        if (DELETE_ME_DEBUG) {
            std::cout << "Roots stored in the heap [AFTER] delete min [BEFORE] consolidate_tree: " << std::endl;
            print_heap();
        }
        ...
        // DEBUG
        if (DELETE_ME_DEBUG) {
            std::cout << "Roots stored in the heap [AFTER] consolidate_tree: " << std::endl;
            print_heap();
        }
    }
        ...
        while (current_index < roots.size()) {
            ...
            if (rank[current_degree].use_count() == 0) {
                ...
                if (rank[current_degree]->priority < roots[current_index]->priority) {
                    ...
                    std::cout << "root stored in [RANK] has lower priority" << std::endl;
                ...
                // than the tree root stored in rank.
                    ...
                    std::cout << "root stored in [ROOTS] has lower priority" << std::endl;
    ...
    */
    ...
    /*
    Get the minimum node in the priority queue, without removing it.

    @return: (std::shared_ptr<fhNode<T>>) - Pointer to the minimum 
                                            node in the collection.
    */
    std::shared_ptr<fhNode<T>> find_min() { return min_node; }
};
